...
\textbf{Solution:} Multi-Signal Fusion Framework combining IoT kitchen sensors, enhanced merchant app, ML pattern detection, and external APIs. Our trained model achieves \textbf{29.9\% improvement} in MAE and \textbf{29.5\% improvement} in P90 error.

\vspace{0.3cm}

\textbf{Expected ROI:} ₹2.7B+ annual savings with improved prediction accuracy across all metrics.
\end{tcolorbox}
...
\begin{table}[H]
\centering
\caption{Current System Performance (Baseline)}
\begin{tabular}{@{}lcc@{}}
\toprule
\textbf{Metric} & \textbf{Value} & \textbf{Target} \\ \midrule
Mean Absolute Error (MAE) & 3.30 minutes & $<$2.5 min \\
Root Mean Squared Error (RMSE) & 4.30 minutes & $<$3.0 min \\
R² Score & 0.9482 & $>$0.96 \\
P50 ETA Error & 2.59 minutes & $<$2.0 min \\
P90 ETA Error & 7.10 minutes & $<$5.0 min \\
P95 ETA Error & 8.96 minutes & $<$6.5 min \\
\bottomrule
\end{tabular}
\label{tab:baseline}
\end{table}

\subsection{Feature Engineering: 5 New Predictive Features}

Beyond the baseline features (merchant marking, time of day, rush hour indicators), we developed 5 new features:

\begin{enumerate}[leftmargin=*]
    \item \textbf{Order Value per Item (Rank \#22, Importance: 0.0013)}
    \begin{itemize}
        \item Higher ratio suggests complex/expensive items requiring more prep time
        \item Formula: $\text{order\_value} / (\text{num\_items} + 1)$
    \end{itemize}
    
    \item \textbf{Distance to Nearest Rush Hour (Rank \#23, Importance: 0.0026)}
    \begin{itemize}
        \item Temporal proximity to peak hours (12pm, 1pm, 7pm, 8pm)
        \item Captures kitchen state transitions during rush buildup/decline
    \end{itemize}
    
    \item \textbf{Kitchen Efficiency Score (Rank \#24, Importance: 0.0005)}
    \begin{itemize}
        \item Load-to-capacity ratio: $\text{estimated\_load} / (\text{capacity} + 1)$
        \item Higher values indicate stressed kitchen conditions
    \end{itemize}
    
    \item \textbf{Order Complexity Score (Rank \#25, Importance: 0.0014)}
    \begin{itemize}
        \item Normalized items and value relative to merchant averages
        \item Captures order deviations from typical patterns
    \end{itemize}
    
    \item \textbf{Merchant Historical Bias (Rank \#2, Importance: 0.0172)}
    \begin{itemize}
        \item Tracks each merchant's systematic over/under estimation pattern
        \item \textit{Second most important feature overall}
        \item Enables merchant-specific bias correction
    \end{itemize}
\end{enumerate}

\textbf{Impact:} The merchant historical bias feature alone shows significant predictive power, ranking as the second most important feature after the merchant marking itself.

\subsection{Improved System Performance}

With Multi-Signal Fusion and 5 new engineered features:
...
MAE & 3.30 min & 2.32 min & \textcolor{zomato}{-29.9\%} & \checkmark \\
RMSE & 4.30 min & 3.08 min & \textcolor{zomato}{-28.3\%} & \checkmark \\
R² Score & 0.9482 & 0.9733 & \textcolor{zomato}{+2.7\%} & \checkmark \\
P50 ETA Error & 2.59 min & 1.78 min & \textcolor{zomato}{-31.3\%} & \checkmark \\
P90 ETA Error & 7.10 min & 5.01 min & \textcolor{zomato}{-29.5\%} & \checkmark \\
P95 ETA Error & 8.96 min & 6.34 min & \textcolor{zomato}{-29.2\%} & \checkmark \\
\bottomrule
\end{tabular}
\label{tab:improved}
\end{table}
...
\begin{tcolorbox}[colback=lightgray,colframe=zomato,title=\textbf{Key Achievement: All Primary Metrics Improved by ~30\%}]

\textbf{✓ MAE:} 3.30 → 2.32 minutes (\textbf{-29.9\%})

\textbf{✓ P90 ETA Error:} 7.10 → 5.01 minutes (\textbf{-29.5\%})

\textbf{✓ RMSE:} 4.30 → 3.08 minutes (\textbf{-28.3\%})

\textbf{✓ P95 Error:} 8.96 → 6.34 minutes (\textbf{-29.2\%})

\textbf{✓ R² Score:} 0.9482 → 0.9733 (\textbf{+2.7\%})
...
\textbf{Based on Model Training Results:}

\textbf{Annual Savings from Prediction Improvement: ₹2.7 Billion}

\begin{itemize}
    \item Reduced rider wait time costs
    \item Improved resource allocation efficiency
    \item Better customer ETA accuracy reducing complaints
    \item Decreased order delays and cancellations
\end{itemize}

\textit{Note: Savings calculated at ₹5 per minute of improved prediction accuracy across 1.5M daily orders.}
...
\begin{itemize}
    ...
    \item Savings: ₹2.7B (assuming 12-month ramp to full benefit)
    \item Net: +₹2.56B (positive from year 1)
\end{itemize}

\textbf{Year 2 onwards:}
\begin{itemize}
    \item Cost: ₹54M/year (recurring only)
    \item Savings: ₹2.7B/year
    \item Net: +₹2.65B/year profit
\end{itemize}

\textbf{Breakeven: 2-3 months}

\textbf{3-Year NPV (10\% discount): ₹6.8B+}

\textbf{5-Year NPV: ₹11.5B+}

\begin{tcolorbox}[colback=lightgray,colframe=darkblue,title=\textbf{ROI Summary}]
\begin{itemize}
    \item Initial investment: ₹85M
    \item Breakeven: 2-3 months
    \item Annual net benefit (steady state): ₹2.65B
    \item 5-year ROI: 13,400\%+
\end{itemize}
\end{tcolorbox}
...
\subsection{Validation: Model Performance}

Training results across different feature sets:

\begin{table}[H]
\centering
\caption{Impact of Enhanced Features and Multi-Signal Approach}
\begin{tabular}{@{}lcc@{}}
\toprule
\textbf{Model Configuration} & \textbf{MAE} & \textbf{Improvement} \\ \midrule
Baseline (Merchant marking only) & 3.30 min & -- \\
+ Enhanced features & 2.82 min & -14.5\% \\
+ Multi-signal fusion & 2.32 min & -29.9\% \\
\bottomrule
\end{tabular}
\end{table}

\textbf{Key Finding:} The combination of new features and multi-signal approach delivers substantial improvements over baseline predictions.
...
\begin{itemize}[leftmargin=*]
    \item MAE reduced by $\geq$25\%
    \item P90 ETA error reduced by $\geq$25\%
    \item Merchant satisfaction $\geq$4.0/5.0
    \item IoT uptime $\geq$95\%
\end{itemize}

\textit{Note: Our trained model already achieves 29.9\% MAE reduction and 29.5\% P90 error reduction, exceeding these targets.}
...
The kitchen prep time prediction problem is fundamentally a \textbf{signal quality problem}. By enriching and diversifying input signals -- especially capturing the previously invisible total kitchen load -- we achieve substantial improvements in prediction accuracy.
...
\begin{itemize}
    ...
    \item \textbf{Validated:} Trained model demonstrates ~30\% improvements across metrics
    \item \textbf{Financially Sound:} 2-3 month ROI, ₹2.65B annual benefit
\end{itemize}

This represents a fundamental rethinking of KPT prediction. At Zomato's scale, it translates to:
...
\textbf{₹2.7B+ annual savings for Zomato}
\end{center}

\vspace{1cm}

\begin{center}
\textbf{\Large Accurate predictions enable better experiences.}
\end{center}
...
\textbf{Prepared by: Atrishman Mukherjee, Ansh Mathur, Supratik Dey}

\textit{Zomathon 2026 - Problem Statement 1}
